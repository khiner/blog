...
export default (
  <div>
    ...
    <Paragraph>{`$H(z) = B(z) = (1 - q_1z^{-1})(1 - q_2z^{-1}) = 1 - (q_1 + q_2)z^{-1} + q_1q_2z^{-2}$,`}</Paragraph>
    <p>
...
plt.grid(True)`}
    ...
    <Paragraph>{`$y(n) = b_0x(n) + b_1x(n-1) + \\cdots + b_Mx(n-M) - a_1y(n-1) - \\cdots - a_Ny(n-N)$.`}</Paragraph>
    <Paragraph>
      JOS provides a{' '}
    ...
    <Paragraph>where, again, $p_i$ is a pole of the filter, $q_i$ is a zero, and $g$ is a scaling constant.</Paragraph>
    <Paragraph>Remember that transfer function in the Stack Overflow answer? Here it is again:</Paragraph>
    <Paragraph>
