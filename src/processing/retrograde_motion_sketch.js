...
class Planet {
  ...
  onSizeChange() {
    this.radius =
      this.p.map(this.radiusUnscaled, 0, SCALE_FACTOR, 0, this.p.width / 2) *
      SCALE_PLANETS
    this.ellipseA = this.p.map(
    ...
    this.orbitSpeed =
      this.ellipseA === 0 ? 0 : this.p.width / 500 / this.ellipseA
  }
  ...
  draw(p, pos, fillColor) {
    ...
    for (let i = 1; i < 3; i++) {
      ...
      fadedColor = fadeColor(this.p, fillColor, 0.6, false)
    }
    p.pop()
  }
