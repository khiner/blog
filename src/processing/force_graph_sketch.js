...
function getBackgroundColor() {
  const parentStyle = window.getComputedStyle(document.getElementById('force-graph-parent'));
...
export default function sketch (p) {
  ...
  p.setup = function () {
    ...
    const toggleDim = p.height / 40, toggleOffset = 5;
  ...
  class Network {
    constructor(dim) {
      ...
      for (let row = 0; row < this.dim; row++) {
        for (let col = 0; col < this.dim; col++) {
          if (row > 0)
            this.setWeight(row, col, row - 1, col, 1);
          if (row < this.dim - 1)
            this.setWeight(row, col, row + 1, col, 1);
          if (col > 0)
            this.setWeight(row, col, row, col - 1, 1);
          if (col < this.dim - 1)
            this.setWeight(row, col, row, col + 1, 1);
        }
      }
    ...
    onSizeChange() {
      for (let i = 0; i < this.numElements; i++) {
        ...
        let col = i % this.dim;
        this.positions[i][0] = p.width * (col / this.dim) + p.random(-this.randomOffset, this.randomOffset);
        this.positions[i][1] = p.height * (row / this.dim) + p.random(-this.randomOffset, this.randomOffset);
      }
    }
  
    setWeight(row, col, otherRow, otherCol, weight) {
      this.weights[row * this.dim + col][otherRow * this.dim + otherCol] = weight;
    }
    ...
    draw() {
      ...
      if (imageToggle.enabled) {
        let w = img.width / this.dim, h = img.height / this.dim;
        if (imagePixels != null) {
          for (let col = 0; col < this.dim - 1; col++) {
            for (let row = 0; row < this.dim - 1; row++) {
              ...
              p.image(imageSection, this.positions[i][0], this.positions[i][1], this.positions[i + 1][0] - this.positions[i][0] + 1, this.positions[i + this.dim][1] - this.positions[i][1] + 1);
            }
          }
        }
      }
